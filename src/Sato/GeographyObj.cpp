...
void GeographyObj::getAssignedModelMatrix(Mtx out_mtx)
{
    ...
    float invMag_z = 1.0f / PSVECMag(&zDir);
    out_mtx[0][2] = invMag_z * zDir.x * this->mScale.z;
    out_mtx[1][2] = invMag_z * zDir.y * this->mScale.z;
    out_mtx[2][2] = invMag_z * zDir.z * this->mScale.z;

    float invMag_x = 1.0f / PSVECMag(&xDir);
    out_mtx[0][0] = invMag_x * xDir.x * this->mScale.x;
    out_mtx[1][0] = invMag_x * xDir.y * this->mScale.x;
    out_mtx[2][0] = invMag_x * xDir.z * this->mScale.x;

    float invMag_y = 1.0f / PSVECMag(&yDir);
    out_mtx[0][1] = invMag_y * yDir.x * this->mScale.y;
    out_mtx[1][1] = invMag_y * yDir.y * this->mScale.y;
    out_mtx[2][1] = invMag_y * yDir.z * this->mScale.y;

    JGeometry::TVec3f position;
    const CrsData::SObject *pObjData = this->mObjData;
    if (!pObjData) {
        position = this->mPos;
    } else {
...
void GeographyObj::resetObject()
{
    ...
    if (GeographyObjManager::gMgr != nullptr) {
        SiUtil::setItemReaction(GeographyObjManager::gMgr->_49C, this);
    }
}
bool GeographyObj::IsHitKart(const JGeometry::TVec3f &spherePos, f32 sphereRadius, JGeometry::TVec3f *posAdjust)
{
    ...
    for (u8 i = 0; i < mBoundsNum; i++, pBounds++) {
        boundsCheck(i, mBoundsNum);
        if ((*pBounds)->IsHitSphere(_13c[i], spherePos, sphereRadius)) {
            if (posAdjust && (_58 != 2)) {
                JGeometry::TVec3f collPos = (*pBounds)->mPos;
                float distance = (*pBounds)->mBoundDepth;
                collPos *= distance;
                *posAdjust += collPos;
            }
            isHit = true;
...
void GeographyObj::setIsHitKartFlg(int kart_index)
{
    ...
    this->mIsHitKartFlags |= (1 << kart_index);
}
...
void GeographyObj::createMultiBoundsPtrAll(u8 param_1)
{
    ...
    this->mBoundsNum = param_1;
    this->mBounds = new ObjColBase *[this->mBoundsNum];
    this->mColObjPos = new JGeometry::TVec3f[this->mBoundsNum];
    this->_13c = new JGeometry::TVec3f[this->mBoundsNum];
    for (u8 i = 0; i < this->mBoundsNum; i++) {
        mColObjPos[i].zero();
        _13c[i].zero();
...
void GeographyObj::calcBoundsGlPos()
{
    ...
    JGeometry::TVec3f *p138 = mColObjPos;
    Mtx srtMtx;
    getSRTMtx(srtMtx);
    JGeometry::TVec3f *p13c = _13c;
    for (u8 i = 0; i < mBoundsNum; i++, p138++, p13c++) {
        PSMTXMultVec(srtMtx, p138, p13c);
    }
}
void GeographyObj::createBoundsSphere(J3DModelData *modelData)
{
    ...
    _13c = new JGeometry::TVec3f(0.0f, 0.0f, 0.0f);
    mBoundsNum++;
...
f32 GeographyObj::getColScaleRadius()
{
    ...
    if (mBounds && radius > 0.0f) {
        radius *= mBounds[_141]->mScale;
    }
    return radius;
}
...
f32 GeographyObj::getAxisMaxScale()
{
    ...
    if (mScale.y > maxScale) {
        maxScale = mScale.y;
    }
    if (mScale.z > maxScale) {
        maxScale = mScale.z;
    }
    return maxScale;
}
