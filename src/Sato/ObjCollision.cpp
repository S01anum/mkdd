...
// UNUSED, size match
ObjColBase::ObjColBase(J3DModelData *modelData, float scale, ObjColBase::CKind kind) : mKind(kind), mScale(scale)
{
    if (!modelData) { return; }
...
void ObjColCube::makeParameter(J3DModelData *modelData, Mtx matrix)
{
    ...
    // edge lengths
    mDimensions[0] = PSVECMag(&edges[0]);
    mDimensions[1] = PSVECMag(&edges[1]);
    mDimensions[2] = PSVECMag(&edges[4]);

    const u8 topIndex = 4;

    JGeometry::TVec3f normal, *edge = edges;
    for (u8 i = 0; i < topIndex; i++, edge++) {
        PSVECCrossProduct(&edges[topIndex], edge, &normal);
...
bool ObjColCube::chkIsHitQuad(const JGeometry::TVec3f &spherePos, const f32 &radius)
{
    ...
    for (u8 i = 0; i < 2; i++) {
        float adjusted = penetrations[closeFaces[i]] - (2.0f * radius);
...
void ExObjColBase::setWallParam(int index)
{
    ...
    JGeometry::TVec3f normal;
    PSVECCrossProduct(&upVector, &mWallNormal, &normal);
    normal.y = 0.0f;
...
void ObjColJump1::makeParameter(J3DModelData *modelData, Mtx matrix)
{
    ...
    for (u8 i = 0; i < 4; i++, edge++) {
        PSVECCrossProduct(&upVector, edge, &normal);
        normal.y = 0.0f;
        stMakePlaneParam(mSidePlanes[i], normal, vertices[i]);
    }

    PSVECCrossProduct(&edges[4], &edges[3], &normal);
    stMakePlaneParam(mTopPlane, normal, vertices[3]);

    PSVECCrossProduct(&edges[1], &edges[0], &normal);
...
void ObjColBlock::makeParameter(J3DModelData *modelData, Mtx matrix)
{
    ...
    PSVECCrossProduct(&edges[0], &edges[1], &normal);
    stMakePlaneParam(mBottomPlane, normal, vertices[1]);
